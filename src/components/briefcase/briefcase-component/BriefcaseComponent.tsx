    ...
    <section>
      ...
      <div className="flex flex-col sm:flex-row  w-full h-screen sm:h-[300px] sm:mt-6">
        ...
        <div  className=" flex flex-col border-4 border-white w-full h-auto  rounded-xl animate-fade-down animate-once animate-duration-[1100ms] ">
          ...
          <button onClick={()=>{setIsOpen(!isOpen);document.getElementById("PORTFOLIO")?.scrollIntoView({ behavior: 'smooth' });}} ><MdClose  className=" text-[35px] mx-2 sm:text-[55px] w-full hover:text-offRed "/></button>
          </div>

          <h1 className="text-[40px] sm:text-start text-center border-b-4 border-offRed mx-4 sm:mx-10">{content?.title}</h1>
          ...
          <div className=" w-full sm:w-[75%] sm:mt-5 sm:ml-5">
          <h3>{content?.description}</h3>
          <div className=" flex flex-row justify-center mt-5 mb-5 sm:mb-10">
          {content?.link &&
